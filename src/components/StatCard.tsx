  ...
  <div className="glass-panel rounded-2xl p-6 border border-border-light/60 dark:border-border-dark/60 hover:shadow-glass-hover transition-shadow h-full flex flex-col justify-between">
    <div className="flex items-center justify-between gap-4 w-full">
      <div className="min-w-0 flex-1">
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-subtext-light dark:text-subtext-dark truncate">
          {label}
        </p>
        <p className="text-3xl font-display font-black text-text-light dark:text-text-dark mt-2 truncate">
          {value}
        </p>
        {hint ? (
          <p className="text-xs font-bold text-subtext-light dark:text-subtext-dark mt-2 truncate">
            {hint}
          </p>
        ) : (
          <p className="text-xs font-bold text-transparent select-none mt-2 truncate">
            &nbsp;
          </p>
        )}
      </div>
